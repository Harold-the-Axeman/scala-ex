...
object Tables {
  ...
  /** Entity class storing rows of table UrlTable
    ...
    *  @param create_time Database column create_time SqlType(TIMESTAMP) */
  case class Url(id: Long, url: String = "", url_hash: String = "", title: String = "", description: String = "", count: Int = 0, owner: Long, is_anonymous: Int = 0, create_time: java.sql.Timestamp)
  ...
  class UrlTable(_tableTag: Tag) extends Table[Url](_tableTag, "url") {
    def * = (id, url, url_hash, title, description, count, owner, is_anonymous, create_time) <> (Url.tupled, Url.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = (Rep.Some(id), Rep.Some(url), Rep.Some(url_hash), Rep.Some(title), Rep.Some(description), Rep.Some(count), Rep.Some(owner), Rep.Some(is_anonymous), Rep.Some(create_time)).shaped.<>({r=>import r._; _1.map(_=> Url.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6.get, _7.get, _8.get, _9.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
