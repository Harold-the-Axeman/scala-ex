...
/** Entity class storing rows of table SystemLogTable
  ...
  *  @param create_time Database column create_time SqlType(TIMESTAMP) */
case class SystemLog(id: Long, user_tag: String = "", log_type: String = "", meta_data: String = "", create_time: java.sql.Timestamp)
...
class SystemLogTable(_tableTag: Tag) extends Table[SystemLog](_tableTag, "system_log") {
  def * = (id, user_tag, log_type, meta_data, create_time) <> (SystemLog.tupled, SystemLog.unapply)
  /** Maps whole row to an option. Useful for outer joins. */
  def ? = (Rep.Some(id), Rep.Some(user_tag), Rep.Some(log_type), Rep.Some(meta_data), Rep.Some(create_time)).shaped.<>({r=>import r._; _1.map(_=> SystemLog.tupled((_1.get, _2.get, _3.get, _4.get, _5.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
...
/** Entity class storing rows of table LocationLogTable
  ...
  *  @param create_time Database column create_time SqlType(TIMESTAMP) */
case class LocationLog(id: Long, user_tag: String = "", address: String = "", create_time: java.sql.Timestamp)
...
class LocationLogTable(_tableTag: Tag) extends Table[LocationLog](_tableTag, "location_log") {
  def * = (id, user_tag, address, create_time) <> (LocationLog.tupled, LocationLog.unapply)
  /** Maps whole row to an option. Useful for outer joins. */
  def ? = (Rep.Some(id), Rep.Some(user_tag), Rep.Some(address), Rep.Some(create_time)).shaped.<>({r=>import r._; _1.map(_=> LocationLog.tupled((_1.get, _2.get, _3.get, _4.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
